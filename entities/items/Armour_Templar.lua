...
ITEM.Name = "Templar Armour"

ITEM.Price = 400
local price = 400

-- The description for this item, used in the inventory menu.
ITEM.Description = "The Armour used by Templars"
...
if ( SERVER ) then -- Only used on server so use this check.
	function ITEM:USE( ply )
		...
		local Armour = ents.Create( "armour_templar" )
		...
		if ply:GetNWInt("templararmour") == 1 then
			...
			ply:SetNWInt("templararmour", 0)
			if ply:GetNWInt("armour") > 99 then
				ply:SetNWInt("armour", ply:GetNWInt("armour") - 100)
			else
				ply:SetNWInt("armour", 0)
			end
			ply:SetNWInt("maxarmour", ply:GetNWInt("maxarmour") - 100)
			...
			elseif ply:GetNWInt("romanshield") == 1 then
				if ply:GetNWString("race") == "Dwarf" then
	...
	function ITEM:EQUIPARMOUR( ply ) -- Equip the item
		if ply:GetNWInt("templararmour") == 0 then
			...
			ply:SetNWInt("templararmour", 1)
			ply:SetNWInt("armour", ply:GetNWInt("armour") + 100)
			ply:SetNWInt("maxarmour", ply:GetNWInt("maxarmour") + 100)
		elseif ply:GetNWInt("templararmour") == 1 then
			ply:ConCommand("pac_removeparts")
			ply:SetNWInt("wearingarmour", 0)
			ply:SetNWInt("templararmour", 0)
			if ply:GetNWInt("armour") > 99 then
