...
function SWEP:PrimaryAttack()
	...
	if self.Owner:KeyDown(IN_FORWARD) or self.Owner:KeyDown(IN_BACK) or self.Owner:KeyDown(IN_MOVELEFT) or self.Owner:KeyDown(IN_MOVERIGHT) then else
		...
		if (SERVER) then
			local Arrow = ents.Create("maw_arrow")
...
function SWEP:SecondaryAttack()
	if self.Owner:KeyDown(IN_SPEED) then
		self.Owner:GiveItem( "maw_bow", 1 )
		InventoryAddon:SendMessage( "You put your Bow in your backpack.", self.Owner, false )
		self.Owner:StripWeapon( "bow" )
	end
end
