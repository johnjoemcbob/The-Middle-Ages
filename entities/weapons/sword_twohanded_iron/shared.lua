...
SWEP.Purpose        	= "A Two-Handed Iron Sword."
...
function SWEP:PrimaryAttack()
	...
	if flevel > 79 then
		...
		timer.Simple(0.75, function() if not self.Owner or not IsFirstTimePredicted() then return end
			self.Weapon:ShootBullets( 40 )
		self.Owner:ViewPunch(Vector(-10, 2, 0)) end)
	else
		self.Owner:PrintMessage(HUD_PRINTTALK, "You need a Fighting Level of 80 to use this weapon.")
	end
end
...
function SWEP:SecondaryAttack()
	if self.Owner:KeyDown(IN_SPEED) then
		self.Owner:GiveItem( "sword_twohanded_iron", 1 )
		InventoryAddon:SendMessage( "You put your Iron Two-Handed Sword in your backpack.", self.Owner, false )
		self.Owner:StripWeapon( "sword_twohanded_iron" )
	end
end
