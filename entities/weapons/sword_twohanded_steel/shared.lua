...
SWEP.Purpose        	= "A Two-Handed Steel Sword."
...
function SWEP:PrimaryAttack()
	...
	if flevel > 89 then
		...
		timer.Simple(0.75, function() if not self.Owner or not IsFirstTimePredicted() then return end
			self.Weapon:ShootBullets( 50 )
		self.Owner:ViewPunch(Vector(-10, 2, 0)) end)
	else
		self.Owner:PrintMessage(HUD_PRINTTALK, "You need a Fighting Level of 90 to use this weapon.")
	end
end
...
function SWEP:SecondaryAttack()
	if self.Owner:KeyDown(IN_SPEED) then
		self.Owner:GiveItem( "sword_twohanded_steel", 1 )
		InventoryAddon:SendMessage( "You put your Steel Two-Handed Sword in your backpack.", self.Owner, false )
		self.Owner:StripWeapon( "sword_twohanded_steel" )
	end
end
