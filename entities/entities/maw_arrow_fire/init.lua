...
function ENT:Initialize()
	...
	self.Entity:Ignite(8)
	self.Entity:DrawShadow(false)
	local phys = self.Entity:GetPhysicsObject()
	local trail = util.SpriteTrail(self.Entity, 0, Color(250,150,0,250), false, 2, 1, 1, 1/(15+1)*0.5, "trails/smoke.vmt")
...
function ENT:PhysicsCollide( data, physobj )  
	...
	if ent:IsNPC() then
		if data.HitEntity:Health() > 10 then
			data.HitEntity:SetHealth(data.HitEntity:Health() - 10)
		else
