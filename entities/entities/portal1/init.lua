...
function ENT:Think()
	...
	for k, ent in pairs(foundents) do
		if ent:IsPlayer() and ent:GetNWInt("portal2x") and ent:GetNWInt("nextport") < CurTime() then
			local xx = ent:GetNWInt("portal2x")
			local yy = ent:GetNWInt("portal2y")
			local zz = ent:GetNWInt("portal2z")
